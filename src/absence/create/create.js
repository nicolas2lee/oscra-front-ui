...
module.exports = function controller(AbsenceService, ActivityTypeService, $scope, $state, MyProfile){
    ...
    $scope.$on('sendAbsence', function(event, absence){
        ...
        var currentUserid = 1;
        AbsenceService.create(absence, absence.absenceType.id, MyProfile.currentUser.id, MyProfile.currentUser.manager.id, MyProfile.currentUser.id,  function (response) {
